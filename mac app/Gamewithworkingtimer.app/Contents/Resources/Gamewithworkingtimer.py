...
if __name__ == "__main__":
    ...
    equation_field = Label(window, textvariable=equation)
    equation_field.grid(columnspan=4, ipadx=70)
    answer_field = Label(window, textvariable=testequation)
    answer_field.grid(columnspan=4, ipadx=70)
    user_score = Label(window, text="score:").grid(row=6, column=0)
    user_score_field = Label(window, textvariable=gamescore).grid(row=6, column=1)
    gametimer = Label(window, text=timeleft)
    gametimer.grid(column=0, row=0)
    high_score = Label(window, text="Highscore:").grid(row=7, column=0)
    high_score_field = Label(window, textvariable=highscore).grid(row=7, column=1)
    ...
    endbutton = Button(
        window, text="End Game", fg="black", command=lambda: [game.end()]
    )
