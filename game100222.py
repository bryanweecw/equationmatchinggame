...
class GameState:
    ...
    def resetGame(self):
        """
        Resets the game, called by the "New Game" button or 
        upon wrong input by user.

        Checks if the int self.score is more than int self.highestscore, 
        then, if true,checks the boolean value of self.congratsplayedbefore

        If self.congratsplayedbefore is false, calls self.highscoresound() 
        to play the new highscore sound, and sets self.congratsplayedbefore 
        to true so that it will not be called again during the same game.

        sets self.testword as an empty string, and calls the self.newWord() 
        function to generate a new string that is assigned to self.testword

        sets self.dispexpress as an empty string, and sets the tkinter string variable
        equation as self.dispexpress, so that the user input is displayed as empty
        to the user in the GUI

        calls self.stopticksound() to stop the ticking sound if it is currently played

        sets int self.timeleft to 30, to reset the timer
        sets int self.turn to -1, to reset the turn/ user input index counter
        """
    ...
    def resetScore(self):
        ...
        Calls scoreReset() tp reset self.score and self.congratsplayedbefore
    ...
    def newWord(self):
        """
        Intialises a local variable probdecider as a boolean value, True
