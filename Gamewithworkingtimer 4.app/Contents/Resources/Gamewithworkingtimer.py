...
if __name__ == "__main__":
    ...
    user_score = tkinter.Label(window, text="Score:").grid(row=6, column=0)
    user_score_field = tkinter.Label(window, textvariable=gamescore).grid(
        row=6, column=1
    )
    gametimer = tkinter.Label(window, text=timeleft, fg="firebrick1")
    gametimer.grid(column=0, row=0)
    high_score = tkinter.Label(window, text="Highscore:").grid(row=7, column=0)
    high_score_field = tkinter.Label(window, textvariable=highscore).grid(
        row=7, column=1
    )
